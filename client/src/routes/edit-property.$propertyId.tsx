...
export const Route = createFileRoute('/edit-property/$propertyId')({
  component: EditPropertyWizard,
})
...
function EditPropertyWizard() {
  ...
  useEffect(() => {
    ...
    // Fetch property and initialize wizard if not already done
    if (propertyId && (!wizardData || wizardData.propertyId !== propertyId)) {
      dispatch(fetchPropertyById(propertyId)).then((action) => {
        if (fetchPropertyById.fulfilled.match(action)) {
          dispatch(initializeWizardForEdit({ property: action.payload, mode: 'edit' }))
        }
      })
    }
  }, [user, propertyId, wizardData, dispatch, navigate])
  ...
  if (!wizardData) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height="100vh">
        <Box>Loading property for editing...</Box>
      </Box>
    )
  }
  ...
  const handleSubmit = async () => {
    try {
      ...
      dispatch(clearWizardData())
      navigate({ to: `/property/${propertyId}`, search: { success: 'Property updated successfully!' } })
    } catch (error) {
