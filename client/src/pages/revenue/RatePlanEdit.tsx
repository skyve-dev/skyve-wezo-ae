...
import { updateRatePlanAsync, fetchRatePlans } from '@/store/slices/ratePlanSlice'

const RatePlanEdit: React.FC = () => {
  const dispatch = useDispatch()
  const { openDialog, registerNavigationGuard, navigateTo } = useAppShell()
  const { ratePlans } = useSelector((state: any) => state.ratePlan)
  
  // Get rate plan ID from URL params (this would be handled by your router)
  // For now, I'll simulate getting it from URL or state
  const [ratePlanId] = useState<string>('rp1') // This would come from useParams() in real implementation
  ...
  useEffect(() => {
    const loadRatePlan = async () => {
      if (ratePlans.length === 0) {
        await dispatch(fetchRatePlans('demo') as any)
      }
      
      const existingPlan = ratePlans.find((plan: any) => plan.id === ratePlanId)
      if (existingPlan) {
        setFormData({
          ...
          includesBreakfast: existingPlan.includesBreakfast,
          percentage: existingPlan.percentage
        })
      }
      setIsLoading(false)
    }
  ...
  const handleSave = async () => {
    setIsSaving(true)
    try {
      await dispatch(updateRatePlanAsync(ratePlanId, formData) as any)
      ...
      await openDialog<void>((close) => (
        <Box padding="2rem" textAlign="center">
          ...
          <Box marginBottom="2rem" color="#374151">
            Rate plan "{formData.name}" has been updated successfully.
          </Box>
          <Button 
            label="Continue"
            onClick={() => close()}
      ...
      await openDialog<void>((close) => (
        <Box padding="2rem" textAlign="center">
          ...
          <Box marginBottom="2rem" color="#374151">
            Failed to update rate plan. Please try again.
          </Box>
          <Button 
            label="Close"
            onClick={() => close()}
      ...
      <SecuredPage>
        <Box display="flex" justifyContent="center" alignItems="center" minHeight="400px">
          <Box textAlign="center">
            ...
            <Box color="#6b7280">Please wait while we load the rate plan details</Box>
          </Box>
        </Box>
      </SecuredPage>
    )
  ...
  return (
    <SecuredPage>
      ...
      >
        ...
        <Box display="flex" justifyContent="space-between" alignItems="start" marginBottom="2rem">
          <Box>
            ...
            <p style={{ 
              ...
              Modify your existing pricing strategy
            </p>
          </Box>
          ...
          <Box display="flex" gap="0.5rem">
            ...
            />
            <Button
              label={window.innerWidth < 480 ? "Save" : "Save Changes"}
              icon={<FaSave />}
              variant="promoted"
              onClick={handleSave}
        ...
        {/* Form Content - Same as Create but pre-populated */}
