...
import {clearError, fetchUserBookings} from '@/store/slices/bookingSlice'
...
const BookingCard: React.FC<BookingCardProps> = ({booking, onCancel, onViewDetails, onWriteReview, userRole}) => {
    const getStatusColor = (status: string) => {
        switch (status?.toLowerCase()) {
            case 'confirmed':
                return {bg: '#dcfce7', color: '#166534', icon: <IoCheckmarkCircle/>}
            case 'pending':
                return {bg: '#fef3c7', color: '#92400e', icon: <IoTime/>}
            case 'cancelled':
                return {bg: '#fee2e2', color: '#dc2626', icon: <IoClose/>}
            case 'completed':
                return {bg: '#e0e7ff', color: '#3730a3', icon: <IoCheckmarkCircle/>}
            default:
                return {bg: '#f3f4f6', color: '#374151', icon: <IoTime/>}
        }
    }
    ...
    return (
        ...
        >
            ...
            <Box display="grid" gridTemplateColumns="1fr" gridTemplateColumnsSm="1fr 1fr" gap="1rem"
                 marginBottom="1.5rem">
